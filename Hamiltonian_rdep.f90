 subroutine Hamiltonian_rdep(np,k,NN,t,rho,energy,wfn,S,b,mass)
   ...
   !.. Input
          ...
          do m = lower, upper
             ...
             do n = 1, k
                if(t(m+1).gt.t(m)) then
                   ...
                   r_1 = r(m,n)
                   term1 = term1 + (2*(rho**2)-r(m,n)**2)*weig(n)*bder(r(m,n),t,k,np,jj+1)*bder(r(m,n),t,k,np,ii+1)
                   call twobody_potential_rdep(S,b,mass,rho,r_1,V)
                   term2 = term2 + weig(n)*bget(r(m,n),t,k,np,jj+1)*bget(r(m,n),t,k,np,ii+1)*V*rho**2.d0
    ...
    return
  end subroutine Hamiltonian_rdep
