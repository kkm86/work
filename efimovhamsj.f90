subroutine efimovham(np,k,L,M,LM,tl,tm,rho,my,r0,d,mass,energy2,H,S)
                    ...
                    if((tl(ll+1).gt.tl(ll)) .and. (tm(mm+1).gt.tm(mm)))then
                       do n = 1, k
                          ...
                          do p = 1, k
                             ...
                             term2 = term2 + weig(n)*weig(p)*4.d0*B_li*dB_mi*B_lj*dB_mj/(((sin(theta))**2.d0)*2.d0*my*(rho**2.d0))
