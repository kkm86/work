 subroutine Hamiltonian_vers_two(np,k,NN,t,rho,energy,S,b,mass,V)
   ...
   !.. Input
    ...
    real(kind(1.d0)) :: sum1, sum2,term1,term2,lower,upper,sumbsp,sumbsp2,bsp,xabsc(k),weig(k),alpha
    integer          :: ii, jj, m, n
          ...
          do m = lower, upper
             ...
             do n = 1, k
                if(t(m+1).gt.t(m)) then
                   ...
                   call lennard_jones(S,b,mass,rho,alpha,V)
                   term2 = term2 + weig(n)*bget(alpha,t,k,np,ii+1)*bget(alpha,t,k,np,jj+1)*V
    ...
    return
  end subroutine Hamiltonian_vers_two
