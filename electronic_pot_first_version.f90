subroutine electronic_pot(np,NN,k,t,xmax,kord,np1,t1,xmax_test,xabsc,weig,RHS,rho,Vexc)
  ...
  real(kind(1.d0)),intent(in) :: xabsc(k),weig(k),t(np),t1(np1),RHS(xmax_test),rho(xmax)
        ...
        do m = lower, upper
           ...
           do n = 1, k
              if(t(m+1).gt.t(m)) then
                 ...
                 potexder2 = baseder2/xval
                 vvv = ABS((-(6.d0*pot/(xval**2.d0))+(6.d0*potex)-(3.d0*potexder2))/(32.d0*(PI**2.d0)))**(1.d0/3.d0)
