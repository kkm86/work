subroutine anisony(np,k,L,M,LM,tl,tm,energy)!,Hama,Sa)
  ...
  use constants
   ...
   !.. Local
   real(kind(1.d0))   :: coordl(L,k),coordm(M,k) !..Abscissa-grid
   real(kind(1.d0))   :: Ham(LM,LM), S(LM,LM), Sl(L,L), Sm(M,M), V_rho(L,L), V_z(M,M), Tll(L,L), Tmm(M,M)
