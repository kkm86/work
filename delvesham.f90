subroutine delvesham(npl,npm,k,L,M,LM,tl,tm,rho,my,r0,d,mass,energy2,H,S)
  ...
  !.. Local
  real(kind(1.d0))   :: coordl(L,k),coordm(M+2,k),xabsc(k),weig(k),theta,alpha
                    ...
                    if((tl(ll+1).gt.tl(ll)) .and. (tm(mm+1).gt.tm(mm)))then
                       do n = 1, k
                          ...
                          do p = 1, k
                             ...
                             call twobody_potential_delves(d,r0,rho,theta,alpha,V)
                            
                           
                             bsp = bsp + weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj

                             term1 = term1 + weig(n)*weig(p)*dB_li*B_mi*dB_lj*B_mj/(2.d0*my*rho**2.d0)
                             term2 = term2 + weig(n)*weig(p)*B_li*dB_mi*B_lj*dB_mj/(((sin(alpha)*cos(alpha))**2.d0)*2.d0*my*rho**2.d0)
                             term3 = term3 + weig(n)*weig(p)*3.d0*B_li*B_mi*B_lj*B_mj/(8.d0*my*rho**2.d0)
                             term4 = term4 + weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*V
                             
                             ! bsp = bsp + weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*sin(theta)*(sin(alpha)*cos(alpha))**2.d0

                             ! term1 = term1 + weig(n)*weig(p)*dB_li*B_mi*dB_lj*B_mj*sin(theta)*(sin(alpha)*cos(alpha))**2.d0/(2.d0*my*rho**2.d0)
                             ! term2 = term2 + weig(n)*weig(p)*B_li*dB_mi*B_lj*dB_mj*sin(theta)/(2.d0*my*rho**2.d0)
                             ! term3 = term3 - weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*sin(theta)*(sin(alpha)*cos(alpha))**2.d0/(8.d0*my*rho**2.d0)
                             ! term4 = term4 + weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*V*sin(theta)*(sin(alpha)*cos(alpha))**2.d0

                             ! bsp = bsp + weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*sin(theta)

                             ! term1 = term1 + weig(n)*weig(p)*dB_li*B_mi*dB_lj*B_mj*sin(theta)/(2.d0*my*rho**2.d0)
                             ! term2 = term2 + weig(n)*weig(p)*B_li*dB_mi*B_lj*dB_mj*sin(theta)/(((sin(alpha)*cos(alpha))**2.d0)*2.d0*my*rho**2.d0)
                             ! term3 = term3 - weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*sin(theta)/(8.d0*my*rho**2.d0)
                             ! term4 = term4 + weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*V*sin(theta)
