subroutine efimovhambound(npl,npm,k,L,M,LM,tl,tm,rho,my,r0,d,mass,energy2,H,S,ny)
                    ...
                    if((tl(ll+1).gt.tl(ll)) .and. (tm(mm+1).gt.tm(mm)))then
                       do n = 1, k
                          ...
                          do p = 1, k
                             ...
                             ! else
                             !    B_mj = bget(coordm(mm,p),tm,k,npm,mj)
                             !    dB_mj = bder(coordm(mm,p),tm,k,npm,mj)
                             ! end if
                             ...
                             ! else
                             !    B_mi = bget(coordm(mm,p),tm,k,npm,mi)
                             !    dB_mi = bder(coordm(mm,p),tm,k,npm,mi)
                             ! end if
                             ...
                             term2 = term2 + weig(n)*weig(p)*8.d0*(ny**2.d0)*B_li*B_mi*B_lj*B_mj*cos(theta)/sin(theta)!/((2.d0*sin(theta)**2.d0)*my*rho**2.d0)
