program efimov
  ...
  integer, parameter :: points = 600
  integer, parameter :: points2 = 0
  real(kind(1.d0))   :: rho_vector(points+points2), energy_curve(7,points+points2)
  ...
  !.. Other parameters
  real(kind(1.d0)) :: rho, my, H(LM,LM), S(LM,LM),t1,t2,s0,lamda, Vtrap(points+points2), angfreq, osc
  ...
  rho_vector(1) = 0.0d0
  rho_vector(points) = 3000.d0
  step_size_x = (rho_vector(1)+rho_vector(points))/points
  do i = 2, points-1
     rho_vector(i) = rho_vector(i-1)+step_size_x
     print*,i, rho_vector(i)
  end do

  ! do i = points2+1, points2+points
  !    rho_vector(i) = rho_vector(i-1)+2.d0*step_size_x
  !    print*, i, rho_vector(i)
  ! end do
...
!   close(20)
  ...
  call CPU_TIME( t1 )
    write(6,*) 'hej5', points
  do i = 1, points+points2
     rho = rho_vector(i)
     WRITE(6,*) "A",I
     call efimovham(npl,npm,k,L,M,LM,tl,tm,rho,my,r0,d(1),mass,energy,H,S,angfreq)
     WRITE(6,*) "b",I
  ...
  open(11,file='result11.dat',status='replace')
  do i = 1, points+points2
     write(11,10)i, rho_vector(i)/osc, energy_curve(1,i)/angfreq, energy_curve(2,i)/angfreq, energy_curve(3,i)/angfreq, energy_curve(4,i)/angfreq, energy_curve(5,i)/angfreq, energy_curve(6,i)/angfreq, Vtrap(i)/angfreq  
