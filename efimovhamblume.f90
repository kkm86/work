subroutine efimovham(npl,npm,k,L,M,LM,tl,tm,rho,my,r0,d,mass,energy2,H,S,angfreq)
  ...
  real(kind(1.d0))   , intent(in) :: tl(npl),tm(npm),rho,my,d,r0,mass(3),angfreq
                    ...
                    if((tl(ll+1).gt.tl(ll)) .and. (tm(mm+1).gt.tm(mm)))then
                       do n = 1, k
                          ...
                          do p = 1, k
                             ...
                             bsp = bsp + weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*sin(4.d0*theta)*(2.d0*my*rho**2.d0)

                             term1 = term1 + weig(n)*weig(p)*1.d0*dB_li*B_mi*dB_lj*B_mj*sin(4.d0*theta)
                             term2 = term2 + weig(n)*weig(p)*2.d0*B_li*dB_mi*B_lj*dB_mj*sin(2.d0*theta)/cos(2.d0*theta)
                             term3 = term3 + weig(n)*weig(p)*15.d0*B_li*B_mi*B_lj*B_mj*sin(4.d0*theta)/4.d0
                             term4 = term4 + weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*sin(4.d0*theta)*(V+0.5d0*my*(angfreq**2.d0)*(rho**2.d0))*(2.d0*my*rho**2.d0)

                             ! term1 = term1 + weig(n)*weig(p)*1.d0*dB_li*B_mi*dB_lj*B_mj/(2.d0*my*rho**2.d0)
                             ! term2 = term2 + weig(n)*weig(p)*1.d0*B_li*dB_mi*B_lj*dB_mj/((cos(2.d0*theta)**2.d0)*2.d0*my*rho**2.d0)
                             ! term3 = term3 + weig(n)*weig(p)*15.d0*B_li*B_mi*B_lj*B_mj/(8.d0*my*rho**2.d0)
                             ! term4 = term4 + weig(n)*weig(p)*B_li*B_mi*B_lj*B_mj*(V+0.5d0*my*(angfreq**2.d0)*(rho**2.d0))
